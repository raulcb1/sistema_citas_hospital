<?php
header('Content-Type: application/json');
include '../config.php';

// Mostrar valores recibidos
file_put_contents('php://stderr', print_r($_GET, true));

// Capturar las fechas de inicio y fin
//$start = isset($_GET['start']) ? $_GET['start'] : null;
//$end = isset($_GET['end']) ? $_GET['end'] : null;


$start = isset($_GET['start']) ? $_GET['start'] : null;
$end = isset($_GET['end']) ? $_GET['end'] : ($start !== null ? $start : null);



// Capturar el servicio a mostrar
$servicio_id = isset($_GET['servicio_id']) ? $_GET['servicio_id'] : null;

// Verificar si las variables start y end están definidas
if ($start === null || $end === null) {
    echo json_encode([
        'success' => false,
        'error' => 'Faltan parametros start o end en la solicitud.',
        'Data' => $start
    ]);
    exit();
}

$sql = "SELECT ac.fecha, ac.turno, s.nombre AS servicio, u.nombre AS medico
    FROM asignacion_consultorios ac
    INNER JOIN servicios s ON ac.servicio_id = s.id
    INNER JOIN medicos m ON ac.medico_id = m.id
    INNER JOIN usuarios u ON m.usuario_id = u.id
    WHERE ac.fecha BETWEEN ? AND ?";

// Agregar filtro por servicio_id si está presente
if ($servicio_id !== null) {
    $sql .= " AND ac.servicio_id = ?";
}

$stmt = $conn->prepare($sql);

// Verificar si la consulta se preparó correctamente
if ($stmt === false) {
    echo json_encode([
        'success' => false,
        'error' => 'Error al preparar la consulta SQL: ' . $conn->error
    ]);
    exit();
}

// Enlazar los parámetros y ejecutar la consulta
if ($servicio_id !== null) {
    $stmt->bind_param("ssi", $start, $end, $servicio_id);
} else {
    $stmt->bind_param("ss", $start, $end);
}

$stmt->execute();
$result = $stmt->get_result();

// Verificar si la consulta se ejecutó correctamente
if ($result === false) {
    echo json_encode([
        'success' => false,
        'error' => 'Error al ejecutar la consulta SQL: ' . $stmt->error
    ]);
    exit();
}

// Construir el array de eventos
$events = [];
/*
while ($row = $result->fetch_assoc()) {
    
    $events[] = [
        'title' => $row['consultorio'],
        'start' => $row['fecha'],
        'allDay' => true,
        'extendedProps' => [
            'consultorio' => $row['consultorio'],
            'medico' => $row['medico'],
            'turno' => $row['turno']
        ]
    ];
}
*/
while ($row = $result->fetch_assoc()) {
    // Determinar el color del evento basado en el turno
    $color = $row['turno'] == 'tarde' ? '#E23D28' : '#3788d8';
    
    $events[] = [
        'title' => $row['servicio'],
        'start' => $row['fecha'],
        'allDay' => true,
        'color' => $color, // Agregar propiedad de color
        'extendedProps' => [
            'servicio' => $row['servicio'],
            'medico' => $row['medico'],
            'turno' => $row['turno']
        ]
    ];
}

// Devolver el JSON de eventos
echo json_encode($events);

// Cerrar la conexión
$stmt->close();
$conn->close();
?>